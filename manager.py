...
def HandleClosed(name, fileAddress):
    print("Wciśnij enter, aby kontynuować...")
    input()
    os.system('cmd /c copy ' + fileAddress + ' PDF\\'+name+".pdf")
...
while True:
    ...
    if choice == '1':  # Pracuj nad kursem
        ...
        if choice2== "0":
            ...
            print('Rozpoczęto pomyślnie. Przed powrotem do programu zamknij okno VIM oraz przeglądarkę PDF.')
            procManager.OpenMasterFile()
            HandleClosed(current_course, 'lic-4\\' + current_course + '\\master.pdf')
    ...
    elif choice == '2':  # Zmień aktywny kurs
